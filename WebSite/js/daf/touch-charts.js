...
(function () {
    ...
    function getDateFilter(dataView, startField, endField, start, end, excluded) {
        ...
        return dataView._combinedFilter([dateFilter].concat(dataView.get_externalFilter().map(function (i) { return i.Name + ':=' + i.Value })));
    }
                    ...
                    if (tabs.css('visibility') == 'hidden') {
                        ...
                        touch.callWithFeedback(button, function () {
                            touch.listPopup({
                                anchor: button,
                                iconPos: 'left',
                                items: calendar.getSupportedViews(dataView).map(function (view) {
                                    return {
                                        text: view,
                                        icon: mode == view.toLowerCase() ? 'check' : false,
    ...
    var Calendar = function (dataView, calendars) {
        ...
        if ($.mobile.activePage.attr('id') != dataView._id)
            config.mode = 'agenda';
        if (!config.mode) {
    ...
    Calendar.prototype = {
        ...
        getSupportedViews: function (dataView) {
            ...
            if (!dataView.tagged('calendar-day-disabled'))
                views.push(resourcesCalendar.Day);
            if (!dataView.tagged('calendar-week-disabled'))
                views.push(resourcesCalendar.Week);
            if (!dataView.tagged('calendar-month-disabled'))
                views.push(resourcesCalendar.Month);
            if (!dataView.tagged('calendar-year-disabled'))
                views.push(resourcesCalendar.Year);
            if (!dataView.tagged('calendar-agenda-disabled'))
                views.push(resourcesCalendar.Agenda);
            return views;
        },
        show: function (options) {
            var that = this,
            ...
            if (calendar.length > 0) {
                ...
                if (!isEcho) {
                    ...
                    $app.touch.tabs('create', {
                        container: tabs,
                        tabs: that.getSupportedViews(dataView).map(function (view) {
                            return {
                                text: view,
                                value: view.toLowerCase(),
                                active: view.toLowerCase() == that.mode
                            };
                        }),
                        change: function (tab) {
