...
(function () {
    ...
    _app.prototype = {
        ...
        delegateCommand: function (command, argument) {
            ...
            if (touch && (command.match(/New/) || command.match(/Select|Edit/) && argument && that._viewId != argument || command == 'Select' && argument && that.editing()) && isForm) {
        ...
        _parseText: function (text, row) {
            ...
            var parent = that.get_parentDataView(that),
                selectedCount = parent._selectedKeyList ? parent._selectedKeyList.length : 0;
        ...
        _invoke: function (methodName, params, onSuccess, userContext, onFailure) {
            ...
            if (servicePath.match(/\.asmx/))
                ...
                _app.odp.invoke(that, {
                    ...
                    cache: false,
                    dataType: 'text',
                    data: _app.htmlEncode(JSON ? JSON.stringify(params) : _serializer.serialize(params))
                }).done(function (result) {
            ...
            if (that._pageIndex < 0 || positionChanged) {
                if (this._pageIndex == -1 || positionChanged) {
                    ...
                    for (i = 0; i < result.Fields.length; i++) {
                        ...
                        if (field.ItemsDataController) {
                            ...
                            if (field._autoAlias) {
                                ...
                                if (field.Items && result.NewRow) {
                                    var newVal = result.NewRow[field.Index];
                                    if (newVal) $(field.Items).each(function (i, v) { if (v[0] == newVal) result.NewRow[lookupFieldAutoAlias.Index] = v[1]; });
                                }
                            }
                        }
                    ...
                    for (i = 0; i < result.Fields.length; i++) {
                        ...
                        if (!String.isBlank(searchOptions) && !(__tf != 4)) {
        ...
        _notifyMaster: function () {
            ...
            if (that.get_hasParent())
                if (_app.touch) {
                    ...
                    if (dv && fieldName) {
                        dv = _app.find(dv._filterSource);
                        field = dv.findField(fieldName);
                        if (field.CausesCalculate)
                            if (dv == _app.touch.dataView())
                                dv._raiseCalculate(field, field);
                            else
                                _app.touch.pageInfo(dv).calculate = fieldName;

                    }
                }
                else {
                    var m = $find(that.get_filterSource());
        ...
        _onExecuteComplete: function (result, context) {
            ...
            if (_app.touch)
                notification = _app.touch.notify(that);
                    ...
                    if (result.NavigateUrl) {
                        ...
                        if (that._closeInstantDetails()) { }
                            ...
                            if (notification != null)
                                _app.touch.notify(notification);
        ...
        showViewMessage: function (message) {
            if (_app.touch)
                _app.touch.notify({ text: message, force: true });
            else {
    ...
    _app.showMessage = function (message) {
        if (_app.touch)
            _app.touch.notify({ text: message, force: true, duration: 'medium' });
        else {
    ...
    */
        ...
        if (m) {
            ...
            if (!String.isNullOrEmpty(hh)) {
                ...
                if (!String.isNullOrEmpty(ampm))
                    ...
                    if (autoAdjustHours != false && !String.isNullOrEmpty(dateTimeFormat.PMDesignator) && dateTimeFormat.ShortTimePattern.indexOf('tt') > 0 && new Date().getHours() >= 12)
                        hh += 12;
                d.setHours(hh);
            }
    ...
    Date.tryParseFuzzyDateTime = function (s, dataFormatString) {
        ...
        if (m) {
            ...
            if (!String.isNullOrEmpty(dateTimeFormat.AMDesignator) && (m[2].toLowerCase() == dateTimeFormat.AMDesignator.toLowerCase() || m[2].toLowerCase() == dateTimeFormat.PMDesignator.toLowerCase()))
    ...
    */
        ...
        function compile() {
            ...
            $(options.topics).each(function (index) {
                ...
                if (categoryVisibleWhen)
                    //result.Expressions.push({ Scope: 2, Target: category.Id, Test: categoryVisibleWhen, Type: 1, ViewId: 'form1' });
    ...
    $(document).on('batcheditsubmit.dataview.app', function (e) {
        ...
        if (focusField) {
            ...
            $app.confirm(resourcesWhenLastCommandBatchEdit.Confirmation).then(function () {
                _app.execute({
                    ...
                    if (_app.touch) {
                        _app.touch.pageShown(function () {
                            ...
                            _app.touch.notify(String.format(resourcesMobile.BatchEdited, result.rowsAffected));
