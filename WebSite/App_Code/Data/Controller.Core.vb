    ...
    Partial Public Class DataControllerBase
        ...
        Public Overloads Shared Function StringToValue(ByVal field As DataField, ByVal s As String) As Object
            If (Not (String.IsNullOrEmpty(s)) AndAlso s.StartsWith("%js%")) Then
                Dim v As Object = JsonConvert.DeserializeObject(s.Substring(4))
        ...
        Public Overridable Function ExecuteList(ByVal requests() As ActionArgs) As ActionResult()
            Dim result As List(Of ActionResult) = New List(Of ActionResult)()
            For Each r As ActionArgs in requests
            ...
            Using connection As DbConnection = CreateConnection()
                ...
                If m_ServerRules.RequiresRowCount Then
                    ...
                    Else
                        ...
                        Else
                            If (page.Rows.Count < page.PageSize) Then
        ...
        Function IDataController_Execute(ByVal controller As String, ByVal view As String, ByVal args As ActionArgs) As ActionResult Implements IDataController.Execute
            ...
            Try 
                ...
                If Not ((args.SqlCommandType = CommandConfigurationType.None)) Then
                    Using connection As DbConnection = CreateConnection()
                        MyCompany.Security.EventTracker.Process(args, m_Config)
                        ...
                        If ((Not (args.SelectedValues) Is Nothing) AndAlso (((args.LastCommandName = "BatchEdit") AndAlso (args.CommandName = "Update")) OrElse ((args.CommandName = "Delete") AndAlso (args.SelectedValues.Length > 1)))) Then
                            ...
                            For Each sv As String in args.SelectedValues
                                ...
                                For Each v As FieldValue in OriginalFieldValues
        ...
        Private Sub EnsureFieldValues(ByVal args As ActionArgs)
            ...
            Dim page As ViewPage = CreateViewPage()
            Dim fieldValues As SortedDictionary(Of String, FieldValue) = New SortedDictionary(Of String, FieldValue)()
            ...
            If (args.Values.Length > 0) Then
                For Each fv As FieldValue in args.Values
                    fieldValues(fv.Name) = fv
                Next
            End If
            Dim missingValues As List(Of FieldValue) = New List(Of FieldValue)()
            For Each f As DataField in page.Fields
            ...
            If (missingValues.Count > 0) Then
                Dim newValues As List(Of FieldValue) = New List(Of FieldValue)(args.Values)
        ...
        Protected Overridable Sub SyncRequestedPage(ByVal request As PageRequest, ByVal page As ViewPage, ByVal connection As DbConnection)
            ...
            Dim keyFields As List(Of DataField) = page.EnumerateSyncFields()
        ...
        Public Overridable Function GetSurvey(ByVal surveyName As String) As String
            Dim root As String = Path.Combine(HttpRuntime.AppDomainAppPath, "js", "surveys")
            Dim survey As String = ControllerConfigurationUtility.GetFileText(Path.Combine(root, (surveyName + ".min.js")), Path.Combine(root, (surveyName + ".js")))
            Dim layout As String = ControllerConfigurationUtility.GetFileText(Path.Combine(root, (surveyName + ".html")), Path.Combine(root, (surveyName + ".htm")))
            Dim rules As String = ControllerConfigurationUtility.GetFileText(Path.Combine(root, (surveyName + ".rules.min.js")), Path.Combine(root, (surveyName + ".rules.js")))
        ...
        Function ExecuteResultSetTable(ByVal page As ViewPage) As DataTable
            ...
            End If
            Dim result As DataTable = resultView.ToTable()
            Dim fieldFilter() As String = page.RequestedFieldFilter()
            If ((Not (fieldFilter) Is Nothing) AndAlso (fieldFilter.Length > 0)) Then
                Dim fieldIndex As Integer = 0
                Do While (fieldIndex < page.Fields.Count)
                    Dim fieldName As String = page.Fields(fieldIndex).Name
                    If ((Array.IndexOf(fieldFilter, fieldName) = -1) AndAlso Not ((fieldName = "group_count_"))) Then
            ...
            End If
            If page.Distinct Then
                Dim groupedTable As DataTable = result.DefaultView.ToTable(true, fieldFilter)
                ...
                For Each r As DataRow in groupedTable.Rows
                    ...
                    For Each fieldName As String in fieldFilter
