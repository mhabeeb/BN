...
Namespace MyCompany.Handlers
    ...
    Public Class BlobHandlerInfo
        ...
        Public ReadOnly Property MaxWidth() As Nullable(Of Integer)
            Get
                Dim m As Match = Regex.Match(Value, "^(\w{2,3})\|")
                Dim size As String = m.Groups(1).Value
                If (size = "tn") Then
                    Return 280
                End If
                If (size = "xxs") Then
        ...
        Private Sub CopyToOutput(ByVal context As HttpContext, ByVal stream As Stream, ByVal handler As BlobHandlerInfo)
            ...
            If handler.AllowCaching Then
                ...
                If ApplicationServices.IsTouchClient Then
                    thumbWidth = 80
                    thumbHeight = 80
                    Dim settings As JObject = CType(ApplicationServices.Create().DefaultSettings("ui")("thumbnail"),JObject)
                    If (Not (settings) Is Nothing) Then
                        If (Not (settings("width")) Is Nothing) Then
                            thumbWidth = Convert.ToInt32(CType(settings("width"),JValue).Value)
                        End If
                        If (Not (settings("height")) Is Nothing) Then
                            thumbHeight = Convert.ToInt32(CType(settings("height"),JValue).Value)
                        End If
                    End If
    ...
    Public Class BlobAdapterFactoryBase
        ...
        Sub ProcessArguments(ByVal controller As String, ByVal fieldName As String, ByVal args As BlobAdapterArguments)
            Dim config As String = ConfigurationManager.AppSettings(String.Format("{0}{1}BlobAdapter", controller, fieldName))
            Dim storageSystem As String = args("storage-system").ToLower()
            If Not (String.IsNullOrEmpty(config)) Then
                Dim configArgs() As String = config.Split(New Char() {Global.Microsoft.VisualBasic.ChrW(59)}, StringSplitOptions.RemoveEmptyEntries)
                For Each arg As String in configArgs
                    Dim parts() As String = arg.Split(New Char() {Global.Microsoft.VisualBasic.ChrW(58), Global.Microsoft.VisualBasic.ChrW(61)}, 2)
                    If (parts.Length = 2) Then
                        args(parts(0).Trim().ToLower()) = parts(1).Trim()
                    End If
                Next
            End If
            Dim replacements As SortedDictionary(Of String, String) = New SortedDictionary(Of String, String)()
            For Each key As String in args.Keys
                Dim value As String = args(key)
            ...
            If Not ((storageSystem = "file")) Then
                Dim keyName As String = "key"
                Dim settingName As String = "AzureBlobStorageKey"
                If (storageSystem = "s3") Then
                    keyName = "access-key"
    ...
    Public Class BlobAdapter
        ...
        Public Shared ReadOnly Property ValidationKey() As String
            Get
                Return "2BFD92930CE9B87B3AAE872CAA375834999775F90FCADA8CD312AC414808CBA1FB1B668AC4539D44A"& _ 
                    "3738CF5845EE4192BD3FAC1B510DE0C4D4E4FAAC0D6BBD0"
            End Get
        End Property
