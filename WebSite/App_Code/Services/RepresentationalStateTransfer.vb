...
Namespace MyCompany.Services
    ...
    Public Class RepresentationalStateTransferBase
        ...
        Overridable Sub IHttpHandler_ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
            ...
            Try 
                If (controllerName = "saas") Then
                    Dim service As String = CType(routeValues("Segment1"),String)
                    Dim handler As OAuthHandler = OAuthHandlerFactory.Create(service)
                ...
                Else
                    ...
                    Else
                        ...
                        Dim isStylesheet As Boolean = (scriptName = "stylesheet")
        ...
        Protected Overridable Sub CombineScripts(ByVal context As HttpContext, ByVal isSaaS As Boolean, ByVal scriptName As String, ByVal culture As String, ByVal version As String)
            ...
            If isSaaS Then
                ...
                Catch __exception As Exception
                End Try
                sb.AppendFormat("var __targetFramework='4.5';__tf=4.0;__cothost='appfactory';__appInfo='BN|{0}';", BusinessRules.JavaScriptString(context.User.Identity.Name))
        ...
        Protected Overridable Sub AuthenticateSaaS(ByVal context As HttpContext)
            ...
            Dim login() As String = JsonConvert.DeserializeObject(Of String())(args)
        ...
        Friend Overridable Sub RenderException(ByVal context As HttpContext, ByVal [error] As Exception, ByVal writer As XmlWriter)
    ...
    Partial Public Class FacebookOAuthHandlerBase
        ...
        Protected Overridable Function GetVersion() As String
            Dim version As String = Config("Version")
        ...
        Public Overrides Sub RequestAuthorizationCode()
            Dim authUrl As String = String.Format("https://www.facebook.com/{0}/dialog/oauth?response_type=code&client_id={1}&redire"& _ 
        ...
        Protected Overrides Function GetQueryRequest(ByVal method As String, ByVal token As String) As WebRequest
            Dim request As WebRequest = WebRequest.Create((("https://graph.facebook.com/" + GetVersion())  _
                            + ("/" + method)))
        ...
        Public Overrides Function GetUserName() As String
            m_UserObj = Query("me?fields=email", false)
    ...
    Partial Public Class GoogleOAuthHandlerBase
        ...
        Protected Overrides ReadOnly Property Scope() As String
            Get
                Dim defaultScope As String = "email"
                If StoreToken Then
        ...
        Public Overrides Sub RequestAuthorizationCode()
            Dim accessType As String = "online"
            If StoreToken Then
                accessType = "offline"
            End If
            Dim authUrl As String = String.Format("https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id={0}&red"& _ 
        ...
        Protected Overrides Function GetAccessTokenRequest(ByVal code As String, ByVal refresh As Boolean) As WebRequest
            Dim request As WebRequest = WebRequest.Create("https://www.googleapis.com/oauth2/v4/token")
            request.Method = "POST"
        ...
        Protected Overrides Function GetQueryRequest(ByVal method As String, ByVal token As String) As WebRequest
            Dim request As WebRequest = WebRequest.Create(("https://www.googleapis.com/" + method))
            request.Headers(HttpRequestHeader.Authorization) = ("Bearer " + token)
            Return request
        End Function
        ...
        Public Overrides Function GetUserRoles(ByVal user As MembershipUser) As List(Of String)
            ...
            Dim result As JObject = Query(("admin/directory/v1/groups?userKey=" + CType(m_UserObj("id"),String)), true)
            If (Not (result) Is Nothing) Then
                For Each group As JObject in CType(result("groups"),JArray)
    ...
    Public Class SharePointOAuthHandlerBase
        ...
        Public Overrides Sub RequestAuthorizationCode()
            ...
            If String.IsNullOrEmpty(version) Then
                version = "15"
            End If
            Dim authUrl As String = String.Format("{0}/_layouts/{1}/OAuthAuthorize.aspx?client_id={2}&scope={3}&response_type=code&r"& _ 
        ...
        Protected Overrides Function GetAccessTokenRequest(ByVal code As String, ByVal refresh As Boolean) As WebRequest
            If String.IsNullOrEmpty(m_Realm) Then
                Dim getRealm As WebRequest = WebRequest.Create((ClientUri + "/_vti_bin/client.svc"))
                ...
                End Try
                Dim wwwAuthentication As String = response.Headers("WWW-Authenticate")
                Dim realmMatch As Match = Regex.Match(wwwAuthentication, "Bearer realm=""(.+?)""")
                m_Realm = realmMatch.Groups(1).Value
            End If
            Dim request As WebRequest = WebRequest.Create((("https://accounts.accesscontrol.windows.net/" + m_Realm)  _
                            + "/tokens/OAuth/2"))
            request.Method = "POST"
            Dim grantType As String = "authorization_code"
            Dim codeName As String = "code"
            If refresh Then
                grantType = "refresh_token"
                codeName = grantType
            End If
            Dim body As String = String.Format("grant_type={0}&client_id={1}%40{2}&client_secret={3}&{4}={5}&redirect_uri={6}&res"& _ 
        ...
        Protected Overrides Function GetQueryRequest(ByVal method As String, ByVal token As String) As WebRequest
            Dim request As WebRequest = WebRequest.Create(String.Format("{0}/_api/{1}", ClientUri, method))
        ...
        Public Overrides Function Query(ByVal method As String, ByVal useSystemToken As Boolean) As JObject
            Dim result As JObject = MyBase.Query(method, useSystemToken)
        ...
        Public Overrides Function GetUserRoles(ByVal user As MembershipUser) As List(Of String)
            Dim roles As List(Of String) = MyBase.GetUserRoles(user)
            Dim groups As JObject = Query(("Web/GetUserById("  _
                            + (CType(m_UserObj("Id"),String) + ")/Groups")), Not (StoreToken))
            If (groups Is Nothing) Then
                Throw New Exception("Unable to get roles.")
            End If
            For Each group As JToken in CType(groups("results"),JArray).Children()
                Dim name As String = CType(group("LoginName"),String)
        ...
        Public Overrides Function GetUserImageUrl(ByVal user As MembershipUser) As String
            Dim info As JObject = Query(String.Format("SP.UserProfiles.PeopleManager/GetPropertiesFor(accountName=@v)?@v='{0}'&$select=P"& _ 
    ...
    Partial Public Class WindowsLiveOAuthHandlerBase
        ...
        Public Overrides Sub RequestAuthorizationCode()
            Dim authUrl As String = String.Format("https://login.live.com/oauth20_authorize.srf?client_id={0}&scope={1}&response_typ"& _ 
        ...
        Protected Overrides Function GetAccessTokenRequest(ByVal code As String, ByVal refresh As Boolean) As WebRequest
            Dim request As WebRequest = WebRequest.Create("https://login.live.com/oauth20_token.srf")
            request.Method = "POST"
            Dim codeName As String = "code"
            Dim grantType As String = "authorization_code"
            If refresh Then
                ...
                grantType = codeName
            End If
            Dim body As String = String.Format("client_id={0}&redirect_uri={1}&client_secret={2}&{3}={4}&grant_type={5}", Config.ClientId, Config.RedirectUri, Config.ClientSecret, codeName, code, grantType)
            Dim bodyBytes() As Byte = Encoding.UTF8.GetBytes(body)
            request.ContentType = "application/x-www-form-urlencoded"
            request.ContentLength = bodyBytes.Length
            Using stream As Stream = request.GetRequestStream()
        ...
        Public Overrides Function GetUserName() As String
            Dim userObj As JObject = Query("me", false)
    ...
    Partial Public Class IdentityServerOAuthHandlerBase
        ...
        Protected Overrides ReadOnly Property Scope() As String
            Get
                Dim configScope As String = Config("Scope")
        ...
        Public Overrides Sub RequestAuthorizationCode()
            Dim authUrl As String = String.Format("{0}/connect/authorize?response_type=code&client_id={1}&redirect_uri={2}&scope={3}"& _ 
                    "&state={4}", ClientUri, Config.ClientId, Config.RedirectUri, Uri.EscapeDataString(Scope), Uri.EscapeDataString(GetState()))
            HttpContext.Current.Response.Redirect(authUrl)
        End Sub
        
        Protected Overrides Function GetAccessTokenRequest(ByVal code As String, ByVal refresh As Boolean) As WebRequest
            Dim request As WebRequest = WebRequest.Create((ClientUri + "/connect/token"))
            request.Method = "POST"
            Dim codeName As String = "code"
            Dim grantType As String = "authorization_code"
            If refresh Then
        ...
        Protected Overrides Function GetQueryRequest(ByVal method As String, ByVal token As String) As WebRequest
            Dim request As WebRequest = WebRequest.Create(String.Format("{0}/connect/{1}", ClientUri, method))
            request.Headers(HttpRequestHeader.Authorization) = ("Bearer " + token)
            Return request
        End Function
        
        Public Overrides Function GetUserName() As String
            Dim userObj As JObject = Query("userinfo", false)
    ...
    Partial Public Class MSGraphOAuthHandlerBase
        ...
        Public Overrides Sub RequestAuthorizationCode()
            Dim sb As StringBuilder = New StringBuilder("https://login.microsoftonline.com/")
            sb.Append(TenantID)
        ...
        Protected Overrides Function GetAccessTokenRequest(ByVal code As String, ByVal refresh As Boolean) As WebRequest
            Dim request As WebRequest = WebRequest.Create(String.Format("https://login.microsoftonline.com/{0}/oauth2/v2.0/token", TenantID))
            request.Method = "POST"
            Dim body As String = String.Format("client_id={0}&redirect_uri={1}&client_secret={2}&scope={3}", Config.ClientId, Uri.EscapeDataString(Config.RedirectUri), Config.ClientSecret, Uri.EscapeDataString(Scope))
            If refresh Then
                body = (body  _
        ...
        Protected Overrides Function GetQueryRequest(ByVal method As String, ByVal token As String) As WebRequest
            Dim request As WebRequest = WebRequest.Create(("https://graph.microsoft.com/v1.0/" + method))
            request.Headers(HttpRequestHeader.Authorization) = ("Bearer " + token)
            Return request
        End Function
        ...
        Protected Overrides Function RefreshTokens(ByVal useSystemToken As Boolean) As Boolean
            ...
            StoreTokens(Tokens)
            StoreToken = false
            Return Not (String.IsNullOrEmpty(Tokens(0)))
        End Function
        ...
        Public Overrides Function GetUserRoles(ByVal user As MembershipUser) As List(Of String)
            Dim roles As List(Of String) = MyBase.GetUserRoles(user)
            Dim roleObj As JObject = Query((("users/" + m_UserID)  _
                            + "/memberOf"), true)
            If (Not (roleObj) Is Nothing) Then
                For Each role As JToken in CType(roleObj("value"),JArray)
